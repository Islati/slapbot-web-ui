<template>
    <v-container fluid class="vue-sound">
        <v-row>
            <!-- Artwork-->
            <v-col cols="1" v-if="image">
                <v-avatar>
                    <v-img :src="image" :alt="title"/>
                </v-avatar>
            </v-col>
            <!-- Player controls -->
            <v-col id="play-pause" cols="1">
                <v-icon @click="togglePlay" :aria-label="playing ? 'pause' : 'play'">{{playing ? 'mdi-pause-circle' : 'mdi-play-circle'}}</v-icon>
            </v-col>
            <v-col cols="8">
                <v-row>
                    <v-col cols="12">
                        <NuxtLink :to="titleLink">{{titleLink}}</NuxtLink>
                    </v-col>
                </v-row>
            </v-col>
        </v-row>
